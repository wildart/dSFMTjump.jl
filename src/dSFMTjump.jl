...
function jump(mt::MersenneTwister, jumppoly::AbstractString)
    ...
    work[end] = zero(UInt64)
    dsfmt = reinterpret(UInt64, copy(mt.state.val))
    dsfmt[end] = UInt64(DSFMT_N*2)
    ...
    nmt = copy(mt)
    work[end] = index
    nmt.state.val = reinterpret(Int32, work)
    Base.Random.mt_setempty!(nmt) # clear cache
    return nmt
end

function add!(dest::Vector{UInt64}, src::Vector{UInt64})
    didx = dest[end]
    sidx = src[end]
...
function next_state!(mts::Vector{UInt64})
    ...
    L0 = mts[u]
    L1 = mts[u+1]
    mts[u]   = (t0 << DSFMT_SL1) $ (L1 >> 32) $ (L1 << 32) $ mts[b]
    mts[u+1] = (t1 << DSFMT_SL1) $ (L0 >> 32) $ (L0 << 32) $ mts[b+1]
    mts[a]   = (mts[u]   >> DSFMT_SR) $ (mts[u]   & DSFMT_MSK1) $ t0
    mts[a+1] = (mts[u+1] >> DSFMT_SR) $ (mts[u+1] & DSFMT_MSK2) $ t1

    mts[end] = (mts[end] + 2) % (DSFMT_N*2)
    return mts
end
